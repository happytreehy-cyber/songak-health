...
async function handleSummary(req, res) {
  ...
  rows.forEach((r, idx) => {
    ...
    if (grade) {
      const gNum = String(grade).replace(/[^0-9]/g, "");
      const gKey = gNum ? gNum + "학년" : grade;
      byGrade[gKey] = (byGrade[gKey] || 0) + 1;
    }
...
async function handleToggleCase(req, res) {
  const { rowNum, eduReport, recoveryReport, returnDate } = req.body;
  if (rowNum === undefined) {
    res.status(400).json({ success: false, message: "대상을 찾을 수 없습니다." });
    return;
  }
  const sheets = getSheetsClient(["https://www.googleapis.com/auth/spreadsheets"]);
  const sheetId = process.env.GOOGLE_SHEET_ID;
  ...
  const data = [];
  if (returnDate !== undefined) data.push({ range: `${SHEET_TAB_NAME}!M${sheetRowNumber}`, values: [[returnDate || ""]] });
  if (eduReport !== undefined) data.push({ range: `${SHEET_TAB_NAME}!N${sheetRowNumber}`, values: [[eduReport ? "Y" : ""]] });
  if (recoveryReport !== undefined) data.push({ range: `${SHEET_TAB_NAME}!O${sheetRowNumber}`, values: [[recoveryReport ? "Y" : ""]] });

  if (!data.length) {
    res.status(400).json({ success: false, message: "변경할 항목이 없습니다." });
    return;
  }
